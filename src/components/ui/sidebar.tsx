  ...
  (
    ...
    return (
      <div
        ...
        data-collapsible={finalCollapsible}
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex flex-col p-2 transition-all", open ? "p-4" : "p-2 items-center", className)}
      {...props}
    />
  )
})
  ...
  (
    ...
    const buttonContent = (
      <div className="flex items-center gap-3">
        {React.Children.toArray(children).find(c => React.isValidElement(c) && (c.type as any).displayName?.includes('Lucide'))}
        <span className={cn("inline-block truncate transition-opacity", open ? "opacity-100" : "opacity-0")}>
          {React.Children.toArray(children).filter(c => !(React.isValidElement(c) && (c.type as any).displayName?.includes('Lucide')))}
        </span>
      </div>
    );
    ...
    const button = (
      <Comp
        ...
        {...props}
      >
        {open ? children : React.Children.toArray(children).find(c => React.isValidElement(c) && (c.type as any).displayName?.includes('Lucide'))}
      </Comp>
    )
