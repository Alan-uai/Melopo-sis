...
| `npx tsx src/scripts/index-nbr.ts` | Index `.txt` and `-research.txt` files into local vector store (RAG) |
...
- **Genkit AI** — `googleai/gemini-2.5-flash` (default). Model fallback via `withFallback()` in `src/ai/genkit.ts` using `MODELS` env var (comma-separated, e.g. `gemini-2.5-flash,gemini-2.5-pro`). Requires `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) in `.env` — copy `.env.example`.
...
- **Vitest** — `environment: 'node'` (no jsdom). `spell-checker.test.ts` mocks `@/lib/dictionary`; `dictionary-integration.test.ts` uses real module.
...
- NBR rules loaded from `docs/nbr/` via `src/lib/nbr-loader.ts` (aumenta o código — has a duplicate `STRUCTURE_FILE_MAP` with `nbr-rag.ts`)
